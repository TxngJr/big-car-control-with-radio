...
void setup() {
  ...
  transmitter.TXaddress("sosad");  // ตั้งชื่อตำแหน่งให้ตรงกัน ชื่อตั้งได้สูงสุด 5 ตัวอักษร
  transmitter.init();
...
void loop() {
  static String prvSenderData;
  String senderData = moveControl() + buttonUDLR() + digitalRead(SWITCH_PIN);
  if (senderData != prvSenderData) {
    Serial.println(senderData);
    prvSenderData = senderData;
    transmitter.txPL(senderData);  // ค่าที่ต้องการส่ง
